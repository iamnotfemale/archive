...
- 소개 · 지금 · 경력 · 학력 · 작업 · 글 · 연락을 한 장으로. 작업 행에 마우스를 올리면 오른쪽 여백에 썸네일과 한 줄
- 오른쪽 위 `PDF ↓`는 인쇄용 레이아웃으로 PDF 저장
- 내용은 코드에 있습니다: `src/content/site.ts`(이름·소개·연락처), `src/content/portfolio.ts`(이력·작업). 이미지는 `public/work/`에 두고 경로를 적으면 됩니다
...
테이블(`items`, `posts`)은 첫 요청 때 자동으로 생성됩니다.
