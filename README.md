...
- `+`로 초안을 만들고 제목·부제목·주소(`/write/<slug>`)·태그·본문을 쓰면 자동 저장 (`Ctrl/⌘+S`로 즉시 저장)
- 아래 줄에서 공개 범위(전체 공개 / 링크만)를 고르고 "발행"을 누르면 바로 발행됩니다. "초안으로"로 되돌릴 수 있습니다
- 본문은 문단 단위이고 작은 마크다운을 이해합니다: `## 소제목`, `> 인용`, `- 목록`, `![캡션](이미지주소)`, `**굵게**`, `[글자](주소)`, 그냥 붙여넣은 주소
- 초안은 열쇠가 있는 브라우저에서만 목록에 보입니다

**portfolio**

- 소개 · 경력 · 학력 · 작업 · 연락을 한 장으로. 작업 행에 마우스를 올리면 오른쪽 여백에 썸네일과 한 줄
...
- 소개 문장, 경력·학력 같은 섹션, 연락처는 오른쪽 위 `&`로 그 자리에서 고칩니다. 손잡이(≡)를 끌어 순서를 바꾸고, "+ 항목" "+ 섹션" "+ 연락처"로 늘리고, ×로 지웁니다. 바뀐 내용은 자동 저장되며 "완료" 또는 Esc로 나옵니다
- 처음 한 번의 기본값만 `src/content/site.ts`(이름·소개·연락처)와 `src/content/portfolio.ts`(경력·학력)에서 옵니다
...
테이블(`items`, `posts`, `works`, `profile`)은 첫 요청 때 자동으로 생성됩니다.
