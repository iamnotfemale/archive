...
- `+`로 초안을 만들고 제목·본문을 쓰면 자동 저장 (`Ctrl/⌘+S`로 즉시 저장)
- "발행"을 누르면 아래에 태그, 공개 범위(전체 공개 / 링크 있는 사람만), 주소(`/write/<slug>`)가 펼쳐집니다
- 본문은 문단 단위이고 작은 마크다운을 이해합니다: `## 소제목`, `> 인용`, `- 목록`, `![캡션](이미지주소)`, `**굵게**`, `[글자](주소)`, 그냥 붙여넣은 주소
- 초안은 열쇠가 있는 브라우저에서만 목록에 보입니다

**portfolio**

- 소개 · 경력 · 학력 · 작업 · 글 · 연락을 한 장으로. 작업 행에 마우스를 올리면 오른쪽 여백에 썸네일과 한 줄
- 작업은 사이트에서 씁니다. 열쇠가 있으면 오른쪽 위 `+`로 만들고, 제목·종류·역할·연도·한 줄 소개·썸네일·본문을 채운 뒤 "발행"에서 주소(`/portfolio/<slug>`)를 정합니다. 본문 마크다운은 글과 같습니다
- 이미지는 편집기의 "이미지 넣기"·"썸네일 올리기"로 올리거나 본문에 붙여넣습니다. 저장소는 Vercel Blob (아래 배포 6번)
...
| 글 수정 | 읽기 화면 왼쪽의 "수정", 또는 편집기 왼쪽 초안 목록 |
| 새 작업 | `/portfolio`에서 오른쪽 위 `+` |
| 작업 수정 | 작업 상세 왼쪽의 "수정", 또는 편집기 왼쪽 작업 목록 |
